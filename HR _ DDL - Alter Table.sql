/* DDL: Alter Table: 기존의 생성된 테이블을 수정
    - 기존 테이블의 컬럼을 추가, 컬럼 이름 변경, 컬럼 삭제
    - 컬럼에 부여된 제약 조건을 생성, 수정, 삭제
*/
...
alter table dept01
add (addr varchar2(200), jumin char(14));

-- 테이블에 컬럼을 추가할 때 null이 기본, update 구문을 사용해서 값을 넣음.
update dept01
set birth = sysdate, email = 'aaa@aaa.com', addr = '서울', jumin = '880520-1111111'
where dno = 10;
commit;

-- 컬럼의 자료형 수정: char, varchar 글자 수를 늘림
alter table dept01
...
rename dept001 to dept01;   -- 다시 원래대로 변경

-- 데이터 사전에서 현재 로그온한 사용자의 모든 테이블을 출력
select * from user_tables;
select table_name from user_tables
order by table_name asc;

/* alter table을 통해서 제약조건 생성, 수정, 삭제
    -- Primary Key
    -- Unique
    -- Foreign Key
    -- Check
...
/* Foreign Key 할당
    ...
    - FK가 적용된 컬럼과 부모 테이블의 참조 컬럼의 자료형이 비슷하거나 같아야 한다.
    - FK가 적용된 컬럼의 값은 부모 테이블의 참조 값이 들어있어야 한다.
*/
-- EMP01 테이블의
...
-- EMP01 테이블의 eno 컬럼에 Primary Key 제약 조건 추가
alter table emp01
add constraint PK_EMP01_ENO primary key (eno);

/* check 조건
    -- 특정 컬럼에 조건에 맞는 값만 저장되도록 설정
...
/* 기존 테이블의  default
    -- default는 제약 조건이 아니다. 제약 조건 이름을 가지지 않는다.
*/

select * from emp01;

-- commission 컬럼의 값을 넣지 않을 경우 기본 값(default): 0
alter table emp01
modify commission default 0;

-- default, null에 값을 넣을 때: default
insert into emp01
values (8001, '고길동', '사무원', 7362, sysdate, 2000, default, 40);

-- 컬럼이 지정되지 않았을 때 default, null
insert into emp01(eno,ename,job,manager,hiredate,salary,dno)
values(8002,'김길동','사무원',7369,sysdate,2000,40);
...
-- 제약 조건 제거: Primary Key: PK_EMP01_ENO
-- 제약 조건 제거: Unipue: U_EMP01_ENAME
-- 제약 조건 제거: Check: CK_EMP01_SALARY
-- 제약 조건 제거: Not Null: NN_EMP01_ENAME
-- 제약 조건 제거: Foreign Key: FK_EMP01_DNO
