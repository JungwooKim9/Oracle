...
/*
     트랜잭션 (Transaction) : 작업(일) 을 처리하는 최소 단위
        ...
        -- 트랜잭션의 4가지 특징
           ...
           - 지속성 (Durability) : commit 트랜잭션은 DB에 저장됨
...
-- dno: where절 사용, 중복된 값이 저장되지 않는 컬럼을 사용(primary key, Unique)
update dept
set dname = 'HR', loc = '대전'
where dno = 40;
...
/*
제약 조건: 컬럼에 부여되는 제약, Primary Key, Unique, Not null, Check, Foreingn Key / Default
    ...
    Primary Key:
        ...
        -- update, delete시 where 절에서 사용되는 컬럼
        -- JOIN시 ON절에서 사용되는 컬럼
...
create table member1 (
    ...
    age number(3),      -- 정수: 3자리
    weight number(5,2)     -- 실수: 전체 5자리 소수점 이하 2자리
);
...
-- update시 where 조건에 Primary key 컬럼을 조건으로 처리

update member1
set addr='부산', phone='010-2222-2222', age=30, weight=88.55
where id='bbb';

-- delete시 where 조건에 primary key 컬럼을 조건으로 처리
delete member1
where id='ccc';
...
-- Insert
insert into member2
values ('bbbb', 'bbbb', '서울', '010-2222-2222', 10, 70.55);
...
-- Insert
insert into member3
values ('bb', 'bb', '서울', '010-1111-1111', 10, 70.55);
...
-- Insert: default에 값 할당: 컬럼이 명시되지 않는 경우 default 값이 자동으로 들어감
...
-- employee 테이블의 dno 컬럼의 값들 할당할 때 department 테이블의 dno 컬럼의 값을 참조 후 할당
...
values (8000, 'KJW', 'MANAGER', 7369, sysdate, 1000, 100, 40);
