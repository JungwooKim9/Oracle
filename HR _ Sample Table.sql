...
-- 부모 테이블은 다른 테이블에서 참조하기 때문에 삭제가 불가능: cascade constrains
...
/*
    ...
    -- NUMBER: Oracle, 정수, 실수
    NUMBER(4): 정수 4자리만 입력 가능
    NUMBER(10,2): 실수, 전체 10자리, 소수점 이하 2자리 입력 가능
...
/* 2개 테이블 조인 */
-- Orders, memberes 테이블의 정보를 join 구문으로 출력
...
-- 각 테이블 조인, orders, product 테이블 조인
...
-- Outer JOIN을 사용해서 구매(Oders)되지 않는 제품이 무엇인지 확인:
select * from orders;
select * from products;

-- EQUI JOIN
select productname, price, quantity, o.productid, p.productid
from orders o, products p
where o.productid(+) = p.productid
